...
function LoginForm() {
  ...
  const handleGoogle = async () => {
    ...
    if (Capacitor.isNativePlatform()) {
      if (!GOOGLE_WEB_CLIENT_ID) {
        setError('Google sign-in is not configured yet. Use email or phone for now.')
        setLoading(false)
        return
      }
      try {
        ...
        const { error } = await supabase.auth.signInWithIdToken({ provider: 'google', token: result.idToken, nonce })
    ...
    const { error } = await supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: `${window.location.origin}/auth/callback?next=${next}` } })
    if (error) {
  ...
  const handlePhoneSend = async () => {
    ...
    if (error) { setError(error.message.replace(/^supabase/i, '').trim()); setLoading(false); return }
  ...
  const handlePhoneVerify = async () => {
    ...
    if (error) { setError(error.message.replace(/^supabase/i, '').trim()); setLoading(false); return }
    track('signed_in', { method: 'phone_otp' }); router.push(next); router.refresh()
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'signin') {
        ...
        if (error) throw error
        track('signed_in', { method: 'email_password' }); router.push(next); router.refresh()
      } else {
        const { error } = await supabase.auth.signUp({ email, password, options: { emailRedirectTo: `${window.location.origin}/auth/callback?next=${next}` } })
        if (error) throw error
        track('signed_up', { method: 'email' }); setMessage('Check your inbox for a confirmation link, then sign in.')
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-ink-50 relative overflow-hidden" style={{ fontFamily: body }}>
      <div className="absolute -top-24 -right-20 w-72 h-72 rounded-full bg-brand-500/10 blur-3xl" /><div className="absolute bottom-0 left-0 w-80 h-80 rounded-full bg-accent-500/10 blur-3xl" />
      <div className="relative max-w-md mx-auto min-h-screen flex flex-col px-6 pt-12 pb-10">
        <Link href="/" className="flex items-center gap-2" aria-label="Nafaka home"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-brand-600 text-white shadow-glow"><Sparkles size={17} /></span><span className="font-display text-xl font-semibold text-ink-900 tracking-tight">Nafaka</span></Link>
        <div className="mt-14 flex-1 flex flex-col justify-center animate-fade-up">
          ...
          {mode !== 'phone' && <button type="button" onClick={() => { setMode('phone'); setError(null); setMessage(null) }} className="cursor-pointer mt-3 text-center text-sm text-ink-500 hover:underline">Use your phone number instead</button>}
        </div>
        <p className="text-center text-xs text-ink-400">Your data is protected with row-level security.</p>
      </div>
    </div>
  )
}
