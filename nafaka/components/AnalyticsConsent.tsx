  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-[100] border-t border-ink-200 bg-white px-4 py-3 shadow-lg md:bottom-4 md:left-4 md:right-auto md:max-w-md md:rounded-2xl md:border">
      ...
      <div className="mt-3 flex gap-2">
        <button type="button" onClick={() => choose(false)} className="flex-1 rounded-xl border border-ink-200 px-3 py-2 text-xs font-semibold text-ink-700 hover:bg-ink-50">
          Decline
        </button>
        <button type="button" onClick={() => choose(true)} className="flex-1 rounded-xl bg-brand-600 px-3 py-2 text-xs font-semibold text-white hover:bg-brand-700">
          Allow analytics
        </button>
      </div>
    </div>
