...
export function AddTransactionModal({
  ...
  const submit = () => {
    if (!amount || !category || done) return
    const parts = [note.trim()]
    if (method !== 'cash') parts.push(`via ${methods.find((m) => m.key === method)?.label}`)
    if (flags.commitment) parts.push('commitment')
    if (flags.discretionary) parts.push('discretionary')
    if (flags.social) parts.push('social')
